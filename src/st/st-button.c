...
static void
button_clicked_cb (ClutterPressGesture *press_gesture,
                   StButton            *button)
{
...
static void
button_notify_pressed_cb (GObject    *gobject,
                          GParamSpec *pspec,
                          gpointer    data)
{
    ...
    case PROP_PRESSED:
      {
        ...
        pressed = clutter_press_gesture_get_pressed (CLUTTER_PRESS_GESTURE (priv->click_gesture));
        g_value_set_boolean (value, pressed);
        break;
      }
...
st_button_init (StButton *button)
{
  ...
  g_signal_connect (priv->click_gesture, "notify::pressed",
                    G_CALLBACK (button_notify_pressed_cb), button);
