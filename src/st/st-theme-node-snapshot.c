...
static void st_theme_node_snapshot_borders (StThemeNodePaintState *state,
  ...
  if (cogl_framebuffer_allocate (framebuffer, &error))
    {
      ...
      g_autoptr (ClutterPaintNode) paint_node = NULL;
      ...
      paint_context = clutter_paint_context_new_for_framebuffer (framebuffer, NULL, CLUTTER_PAINT_FLAG_NONE);
      snapshot = clutter_snapshot_new (paint_context);

      st_theme_node_snapshot_borders (state, snapshot, &box, ST_PAINT_BORDERS_MODE_SILHOUETTE, 0xFF);

      paint_node = clutter_snapshot_free_to_node (snapshot);
      clutter_paint_node_paint (paint_node, paint_context);
...
static void
snapshot_material_with_opacity (CoglPipeline    *material,
                                ClutterSnapshot *snapshot,
                                ClutterActorBox *box,
                                ...
                                guint8           paint_opacity)
{
  ...
  clutter_snapshot_push_pipeline (snapshot, material);
  if (coords)
    clutter_snapshot_add_texture_rectangle (snapshot, box,
...
static void
st_theme_node_snapshot_borders (StThemeNodePaintState *state,
                                ClutterSnapshot       *snapshot,
                                const ClutterActorBox *box,
                                StPaintBordersMode     mode,
                                guint8                 paint_opacity)
      ...
      if (alpha > 0)
        {
          ...
          clutter_snapshot_push_color (snapshot, &pipeline_color);
          clutter_snapshot_add_rectangles (snapshot, rects, 4);
          clutter_snapshot_pop (snapshot);
        }
    }
      ...
      for (corner_id = 0; corner_id < 4; corner_id++)
        {
          ...
          clutter_snapshot_push_pipeline (snapshot, state->corner_material[corner_id]);
          switch (corner_id)
            {
              case ST_CORNER_TOPLEFT:
                clutter_snapshot_add_texture_rectangle (snapshot,
                                                        &(ClutterActorBox) {
                                                          .x1 = 0,
                                                          .y1 = 0,
                                                          .x2 = max_width_radius[ST_CORNER_TOPLEFT],
                                                          .y2 = max_width_radius[ST_CORNER_TOPLEFT],
                                                        },
                                                        0.0, 0.0, 0.5, 0.5);
                break;
              case ST_CORNER_TOPRIGHT:
                clutter_snapshot_add_texture_rectangle (snapshot,
                                                        &(ClutterActorBox) {
                                                          .x1 = width - max_width_radius[ST_CORNER_TOPRIGHT],
                                                          .y1 = 0,
                                                          .x2 = width,
                                                          .y2 = max_width_radius[ST_CORNER_TOPRIGHT],
                                                        },
                                                        0.5, 0.0, 1.0, 0.5);
                break;
              case ST_CORNER_BOTTOMRIGHT:
                clutter_snapshot_add_texture_rectangle (snapshot,
                                                        &(ClutterActorBox) {
                                                          .x1 = width - max_width_radius[ST_CORNER_BOTTOMRIGHT],
                                                          .y1 = height - max_width_radius[ST_CORNER_BOTTOMRIGHT],
                                                          .x2 = width,
                                                          .y2 = height,
                                                        },
                                                        0.5, 0.5, 1.0, 1.0);
              ...
              case ST_CORNER_BOTTOMLEFT:
                clutter_snapshot_add_texture_rectangle (snapshot,
                                                        &(ClutterActorBox) {
                                                          .x1 = 0,
                                                          .y1 = height - max_width_radius[ST_CORNER_BOTTOMLEFT],
                                                          .x2 = max_width_radius[ST_CORNER_BOTTOMLEFT],
                                                          .y2 = height
                                                        },
                                                        0.0, 0.5, 0.5, 1.0);
                break;
  ...
  if (alpha > 0)
    {
      ...
      for (corner_id = 0; corner_id < 4; corner_id++)
        {
          ...
          clutter_snapshot_add_rectangles (snapshot, verts, n_rects);
        }
      ...
      if (max_border_radius > border_width[ST_SIDE_TOP])
          clutter_snapshot_add_rectangle (snapshot,
                                          &(ClutterActorBox) {
                                            .x1 = MAX (max_border_radius, border_width[ST_SIDE_LEFT]),
                                            .y1 = border_width[ST_SIDE_TOP],
                                            .x2 = width - MAX (max_border_radius, border_width[ST_SIDE_RIGHT]),
                                            .y2 = max_border_radius,
                                          });
      if (max_border_radius > border_width[ST_SIDE_BOTTOM])
          clutter_snapshot_add_rectangle (snapshot,
                                          &(ClutterActorBox) {
                                            .x1 = MAX (max_border_radius, border_width[ST_SIDE_LEFT]),
                                            .y1 = height - max_border_radius,
                                            .x2 = width - MAX (max_border_radius, border_width[ST_SIDE_RIGHT]),
                                            .y2 = height - border_width[ST_SIDE_BOTTOM],
                                          });

      clutter_snapshot_add_rectangle (snapshot,
                                      &(ClutterActorBox) {
                                        .x1 = border_width[ST_SIDE_LEFT],
                                        .y1 = MAX (border_width[ST_SIDE_TOP], max_border_radius),
                                        .x2 = width - border_width[ST_SIDE_RIGHT],
                                        .y2 = height - MAX (border_width[ST_SIDE_BOTTOM], max_border_radius),
                                      });
      clutter_snapshot_pop (snapshot);
    }
}
...
static void
st_theme_node_snapshot_sliced_shadow (StThemeNodePaintState *state,
                                      ClutterSnapshot       *snapshot,
                                      const ClutterActorBox *box,
                                      guint8                 paint_opacity)
{
  ...
  clutter_snapshot_push_pipeline (snapshot, state->box_shadow_pipeline);
  clutter_snapshot_add_texture_rectangles (snapshot, rectangles, idx / 8);
  clutter_snapshot_pop (snapshot);
  ...
  clutter_snapshot_add_rectangle (snapshot,
                                  &(ClutterActorBox) {
                                    .x1 = xend,
                                    .y1 = yoffset,
                                    .x2 = xend + shadow_width,
                                    .y2 = yoffset + shadow_height,
                                  });
  clutter_snapshot_pop (snapshot);
  ...
  clutter_snapshot_add_rectangle (snapshot,
                                  &(ClutterActorBox) {
                                    .x1 = xend,
                                    .y1 = yoffset + shadow_height,
                                    .x2 = xend + shadow_width,
                                    .y2 = yoffset + shadow_height + 1,
                                  });
  clutter_snapshot_add_rectangle (snapshot,
                                  &(ClutterActorBox) {
                                    .x1 = xend,
  ...
  clutter_snapshot_add_rectangle (snapshot,
                                  &(ClutterActorBox) {
                                    .x1 = xend + shadow_width,
                                    .y1 = yoffset,
                                    .x2 = xend + shadow_width + 1,
                                    .y2 = yoffset + shadow_height,
                                  });
...
static void
st_theme_node_snapshot_sliced_border_image (StThemeNode     *node,
                                            ClutterSnapshot *snapshot,
                                            float            width,
                                            float            height,
                                            guint8           paint_opacity)
{
...

  {
    ...
    clutter_snapshot_add_texture_rectangles (snapshot, rectangles, 9);
    clutter_snapshot_pop (snapshot);
  }
}

static void
st_theme_node_snapshot_outline (StThemeNode           *node,
                                ClutterSnapshot       *snapshot,
                                const ClutterActorBox *box,
                                guint8                 paint_opacity)
...
void
st_theme_node_snapshot (StThemeNode           *node,
  ...
  if (state->box_shadow_pipeline)
    {
      if (state->alloc_width < node->box_shadow_min_width ||
          state->alloc_height < node->box_shadow_min_height)
        _st_snapshot_shadow_with_opacity (node->box_shadow,
                                          snapshot,
                                          state->box_shadow_pipeline,
                                          &allocation,
                                          paint_opacity);
      else
        st_theme_node_snapshot_sliced_shadow (state,
                                              snapshot,
                                              &allocation,
                                              paint_opacity);
    }
  ...
  if (state->prerendered_pipeline != NULL ||
      st_theme_node_load_border_image (node, resource_scale))
    {
      if (state->prerendered_pipeline != NULL)
        {
          ...
          snapshot_material_with_opacity (state->prerendered_pipeline,
                                          snapshot,
                                          &paint_box,
                                          NULL,
                                          paint_opacity);
      ...
      if (node->border_slices_pipeline != NULL)
        st_theme_node_snapshot_sliced_border_image (node, snapshot, width, height, paint_opacity);
    }
  else
    {
      st_theme_node_snapshot_borders (state, snapshot, box, ST_PAINT_BORDERS_MODE_COLOR, paint_opacity);
    }

  st_theme_node_snapshot_outline (node, snapshot, box, paint_opacity);
  ...
  if (state->prerendered_pipeline == NULL &&
      st_theme_node_load_background_image (node, resource_scale))
    {
      ...
      if (has_visible_outline || node->background_repeat)
        {
          ...
          clutter_snapshot_add_rectangle (snapshot, &allocation);
        }
      ...
      if (node->background_shadow_pipeline != NULL)
        _st_snapshot_shadow_with_opacity (node->background_image_shadow,
                                          snapshot,
                                          node->background_shadow_pipeline,
                                          &background_box,
                                          paint_opacity);

      snapshot_material_with_opacity (node->background_pipeline,
                                      snapshot,
                                      &background_box,
                                      &texture_coords,
                                      paint_opacity);
